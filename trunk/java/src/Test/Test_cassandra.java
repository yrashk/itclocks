...
public class Test_cassandra implements Runnable{
	...
	public void start() {
		try {
			...
			for(File f : files) {
				if(f.getName().startsWith(TEST_NAME+number_processes+ "_" + number_processes_real + "_v2_" + ratio_loss)) {
					number_test++;
				}
			}
			...
			runs = new File(PATH + File.separator + TEST_NAME + number_processes + "_" + number_processes_real + "_v2_" + ratio_loss + "_" + number_test + "_" + time + ".dat");
		} catch(Exception e) {
			e.printStackTrace();
		}
	}
...
//		for(int i = 0 ; i< 50 ; i++) {
//			Thread t = new Thread(new Test_cassandra(8,8,100000,0.0f));
...
//			for(File f : files) {
//				if(f.getName().startsWith(TEST_NAME2+ "8_8_v2_0")) {
...
//
//
//			File resultado = new File(PATH2+ File.separator +"itc_test_cass_8_8_0.0.dat");
//
//			// Create file
